<script lang="ts">
  ...
  let closestIndex = $derived(is_tracklist_playing ? audioStore.currentSongIndex : currentSongIndex);
...
<style lang="scss">
  ul {
    ...
    li {
      scroll-margin-top: calc(var(--tracklist-height) / 2 - var(--nav-element-spacing-vertical) * 2);
      width: 100%;
      text-align: center;
