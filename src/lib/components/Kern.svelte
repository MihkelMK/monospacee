<script lang="ts">
  ...
  let { silmaNurk = $bindable(0), suu = '◡', paremSilm = "'", vasakSilm = "'", og = false }: Props = $props();
...
<span class="maskott {audioStore?.isPlaying ? 'playing' : ''}">
  ...
  <span class="kern">
    ...
    <span class="kern_suu" style={og ? 'display: block; font-size: 0.8em; transform: rotate(90deg) translate(75%, 6%);' : ''}>
      {og ? ')' : suu}
    </span>
